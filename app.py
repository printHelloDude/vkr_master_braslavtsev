...
Версия: 4.0.1 — Исправлен просмотр утвержденных версий, убрана плашка QC
...
def init_session_state():
    ...
    defaults = {
        ...
        'viewing_approved_version': False
    }
...
def design_page():
                ...
                with st.container(border=True):
                    ...
                    with col3:
                        if st.button("️ Просмотр", key=f"view_{ts.get('id')}", use_container_width=True):
                            ...
                            st.session_state.viewing_approved_version = (ts.get('status') == 'approved')
                            st.rerun()
    ...
    # ДЕТАЛИ ТЗ — показ карточки с файлами (РАБОТАЕТ И ДЛЯ УТВЕРЖДЕННЫХ!)
    if st.session_state.get('selected_ts'):
        ts = st.session_state.selected_ts
        is_approved = ts.get('status') == 'approved'
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.info(f"**Создана:** {ts.get('created_at', 'N/A')}\n\n**Автор:** {ts.get('author', 'N/A')}")
        with col2:
            if is_approved:
                st.success(f"**Утверждена:** {ts.get('approved_at', 'N/A')}\n\n**Статус:** ✅ Утверждена")
            else:
                st.warning(f"**Статус:** 📝 Черновик")
        
        # Показываем файлы ВСЕГДА (даже для утвержденной версии!)
        ...
        if ts.get('documents'):
            for i, doc in enumerate(ts['documents']):
                with st.container(border=True):
                    ...
                    with col1:
                        st.markdown(f"**{doc.get('type', 'Document')}**")
                        st.caption(f"📄 {doc.get('filename', 'unknown')} | {doc.get('size', 0) / 1024:.1f} KB")
        ...
        # Блокировка редактирования ТОЛЬКО для утвержденной версии
        if is_approved:
            st.error("🔒 Утвержденную версию нельзя редактировать. Создайте новую версию для внесения изменений.")
        
        # Кнопка закрытия
        if st.button("← Закрыть", key=f"close_{ts.get('id')}"):
            st.session_state.selected_ts = None
            st.session_state.viewing_approved_version = False
            st.rerun()
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Выйти", use_container_width=True):
            ...
            st.session_state.current_user = None
            st.rerun()
        st.caption("Версия: 4.0.1 (ИСПРАВЛЕНИЯ)")
